...
import { getAccessById } from 'apiSdk/accesses';
import { AccessInterface } from 'interfaces/access';

function AccessViewPage() {
  const { hasAccess } = useAuthorizationApi();
  const router = useRouter();
  const id = router.query.id as string;
  const { data, error, isLoading, mutate } = useSWR<AccessInterface>(
    () => (id ? `/accesses/${id}` : null),
    () =>
      getAccessById(id, {
            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              <Flex alignItems="center" w="full" justifyContent={'space-between'}>
                ...
                {hasAccess('access', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                  <NextLink href={`/accesses/edit/${id}`} passHref legacyBehavior>
                    <Button
              ...
              >
                <FormListItem label="Access Type" text={data?.access_type} />
