...
import { getAccesses } from 'apiSdk/accesses';
...
function DashboardPage() {
  ...
  const accessStat = useSWR<PaginatedInterface<AccessInterface>>(
    () => `/accesses`,
    () => getAccesses({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Accesses',
      result: accessStat?.data?.totalCount,
      isLoading: accessStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
