...
type ColumnType = ColumnDef<DocumentInterface, unknown>;
...
export function DocumentListPage(props: DocumentListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getDocuments({
        relations: ['company', 'user', 'access.count'],
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<DocumentInterface>>(
    () => `/documents?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: DocumentInterface) => {
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('access', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'access',
          header: 'Access',
          accessorKey: 'access',
          cell: ({ row: { original: record } }: any) => record?._count?.access,
        }
      : null,
